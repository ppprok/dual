...
#include "p2c-1.21alpha2\src\p2c.h"
...
Static boolean nohelp, checksim, exinput, exoutput, genoutput, suppressoutput,
...
Static Char inf_NAME[_FNSIZE];
Static Char outf_NAME[_FNSIZE];
...
Static Void read_switches()
{
  ...
  if (P_argc != 1) {
    count = 0;
    while (count != P_argc - 1) {
      count++;
      if (!strcmp(strcpy(STR1, P_argv[count]), "-h"))
	nohelp = false;
      /*Output will be in */
      if (!strcmp(strcpy(STR1, P_argv[count]), "-s1"))   /*generalized form*/
	outputscenario = 1;
      if (!strcmp(strcpy(STR1, P_argv[count]), "-s2"))   /*regular form*/
	outputscenario = 2;
      if (!strcmp(strcpy(STR1, P_argv[count]), "-s3"))   /*sorted regular*/
	outputscenario = 3;
      if (!strcmp(strcpy(STR1, P_argv[count]), "-s4"))   /*just counting it*/
	outputscenario = 4;
      /*Input name*/
      if (!strcmp(strcpy(STR1, P_argv[count]), "-i")) {
	if (*strcpy(STR2, P_argv[count+1]) != '\0') {
	  count++;
	  exinput = true;
	  strcpy(inname, P_argv[count]);
	}
      }
      /*Output name*/
      if (!strcmp(strcpy(STR1, P_argv[count]), "-o")) {
	if (*strcpy(STR2, P_argv[count+1]) != '\0') {
	  count++;
	  exoutput = true;
	  strcpy(outname, P_argv[count]);
	}
      }
      if (!strcmp(strcpy(STR1, P_argv[count]), "-sp")) {
	if (*strcpy(STR2, P_argv[count+1]) != '\0') {
	  count++;
	  sscanf(strcpy(STR3, P_argv[count]), "%ld", &viewstep);
	}
      }
      if (!strcmp(strcpy(STR1, P_argv[count]), "-c"))
	checksim = true;
      if (!strcmp(strcpy(STR1, P_argv[count]), "-p"))
	outputinstance = true;
      if (!strcmp(strcpy(STR1, P_argv[count]), "-q"))
	suppressoutput = true;
      if (strcmp(strcpy(STR1, P_argv[count]), "-sort"))
	continue;
      if (*strcpy(STR2, P_argv[count+1]) != '\0') {
	count++;
	strcpy(sortmode, P_argv[count]);
...
trannode *trans;
{
  ...
  while (tempt != NULL) {
    ...
    FORLIM = n;
    for (i = 1; i <= FORLIM; i++) {
      if (P_inset((int)i, tempt->node->vari)) {
	fprintf(outf, "%3lu,", i);
	fcount++;
      }
    }
...
Static Void print_models(instance, n, m)
setnode *instance;
unsigned long n, m;
{
  unsigned long i, j;
...
Static Void read_instance(instance, n, c)
setnode **instance;
unsigned long *n, *c;
{
...
Static Void check_simplicity(instance)
setnode *instance;
{
  setnode *ip = instance;
  setnode *jp;
...
setnode **instance;
{
  ...
  for (j = n; j >= 1; j--) {
    ...
    temp = *instance;
    while (temp != NULL) {
      if (!P_inset((int)j, temp->vari)) {
	if (previoustemp != NULL)
	  previoustemp->next = temp;
	if (P_inset((int)j, (*instance)->vari) && previoustemp == NULL)
	  *instance = temp;
	previoustemp = temp;
	temp = temp->next;
	continue;
...
Static Void assign_new_tvalue(ttoken, tvalue)
tokennode **ttoken;
long *tvalue;
{
  if (token_space == NULL)
...
Static Void eq_tokens(new_token, old_token)
tokennode **new_token, **old_token;
{
  *new_token = *old_token;
  (*old_token)->links++;
}  /*eq_tokens*/
...
Static Void dispose_transversal(tp, endtp)
trannode **tp, **endtp;
{
  trannode *temptp;
...
Static Void expand_and_store(tp, models)
trannode **tp;
setnode **models;
{
  short a[maxv][maxv + 1];
  ...
  while (tempt != NULL) {
    ...
    FORLIM = n;
    for (j = 1; j <= FORLIM; j++) {
      if (P_inset((int)j, tempt->node->vari)) {
	length++;
	a[imax-1][length] = j;
      }
    }
...
Local Void print_trans(t, LINK)
trannode *t;
struct LOC_add_next_clause *LINK;
{
  trannode *temptp = t;
  ...
  while (temptp != NULL) {
    ...
    for (ii = 1; ii <= FORLIM; ii++) {
      if (P_inset((int)ii, temptp->node->vari))
	printf("%3lu", ii);
    }
    printf(" II ");
    temptp = temptp->next;
...
Local Void add_monogamic(cl, LINK)
setnode **cl;
struct LOC_add_next_clause *LINK;
{
...
Local Void remove_monogamic(rem_clause, LINK)
dsetnode **rem_clause;
struct LOC_add_next_clause *LINK;
{
  dsetnode *tt;
...
Local boolean generate_next_transversal(LINK)
struct LOC_add_next_clause *LINK;
{
  /*mem*/
