...
void Antichain::Register() {
    Registrator<IPosetPtr>::Instance().RegisterFactory("Antichain", []() { return std::make_shared<Antichain>(); });
}
...
void Antichain::SelectUnique(PosetItems& items) const {
    ...
    auto end = items.end();
    auto pos = std::unique(items.begin(), end, [](PosetItem const& a, PosetItem const& b) {
...
void Antichain::TopologicalSort(PosetItems& items) const {
    sort(items, [](PosetItem const& a, PosetItem const& b) { return a.value.vertex > b.value.vertex; });
}
...
bool Antichain::PrecImmediate(PosetItem const& a, PosetItem const& b) const {
    if (_size > 0) {
        return (IsUniversalMinimal(a) && !IsUniversalMaximal(b)) || (IsUniversalMaximal(a) && !IsUniversalMinimal(b));
    }
