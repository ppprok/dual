...
void GraphPoset::Register() {
    Registrator<IPosetPtr>::Instance().RegisterFactory("GraphPoset", []() { return std::make_shared<GraphPoset>(); });
}
...
void GraphPoset::TopologicalSort(PosetItems& items) const {
    std::sort(items.begin(), items.end(), [this](PosetItem const& a, PosetItem const& b) { return TopoLess(a, b); });
}
...
bool GraphPoset::IsTopologicalOrdered(PosetItems const& items) const {
    return std::is_sorted(items.begin(), items.end(), [this](PosetItem const& a, PosetItem const& b) {
        return TopoLess(a, b);
    });
}
