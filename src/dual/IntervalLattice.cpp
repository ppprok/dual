...
void IntervalLattice::Register() {
    Registrator<IPosetPtr>::Instance().RegisterFactory("IntervalLattice",
                                                       []() { return std::make_shared<IntervalLattice>(); });
}
...
bool IntervalLattice::Equal(PosetItem const& left, PosetItem const& right) const {
    ...
    return left.value.interval.min == right.value.interval.min && left.value.interval.max == right.value.interval.max;
}
...
bool IntervalLattice::Prec(PosetItem const& left, PosetItem const& right) const {
    ...
    return right.value.interval.min <= left.value.interval.min && left.value.interval.max <= right.value.interval.max;
}
...
void IntervalLattice::GetAllItems(PosetItems& items) const {
    ...
    for (int i = _universalMaximal.value.interval.min; i < _universalMaximal.value.interval.max; ++i) {
        for (int j = i + 1; j <= _universalMaximal.value.interval.max; ++j) {
...
PosetItem IntervalLattice::FromString(char const* begin, char const* end) const {
    ...
    auto begin1 = std::find_if(begin, end, [](char c) { return c == '[' || c == '('; });
    if (begin1 != end) {
        ++begin1;
        auto end1 = std::find(begin1, end, ':');
        if (end1 != end) {
            auto begin2 = end1 + 1;
            auto end2 = std::find_if(begin2, end, [](char c) { return c == ']' || c == ')'; });
            if (end2 != end)
                return PosetItem(BoundFromString(begin1, end1), BoundFromString(begin2, end2));
        }
    }
...
void IntervalLattice::GetMinimalIndependent(PosetItems const& B, PosetItems& minimal) const {
    ...
    for (int i = _universalMaximal.value.interval.min; i < _universalMaximal.value.interval.max;) {
        bool changed = false;
        for (auto& a : B) {
            if (a.value.interval.min <= i && i + 1 <= a.value.interval.max) {
...
void IntervalLattice::BuildMinimalIndependent(PosetItem current,
                                              PosetItems::const_iterator begin,
                                              PosetItems::const_iterator end,
                                              PosetItems& result) const {
...
bool IntervalLattice::PrecImmediate(PosetItem const& a, PosetItem const& b) const {
...
PosetItem IntervalLattice::Span(PosetItem const& i1, PosetItem const& i2) const {
    ...
    return PosetItem(std::min(i1.value.interval.min, i2.value.interval.min),
...
PosetItem IntervalLattice::Join(PosetItem const& i1, PosetItem const& i2) const {
    ...
    if (i2.value.interval.min <= i1.value.interval.min && i1.value.interval.min < i2.value.interval.max)
        return PosetItem(i1.value.interval.min, i2.value.interval.max);

    if (i1.value.interval.min <= i2.value.interval.min && i2.value.interval.min < i1.value.interval.max)
