...
void PosetsProduct::SetOptions(Options const& options) {
    ...
    for (auto& p : options.get_child("columns")) {
        ...
        auto poset = Registrator<IPosetPtr>::Instance().Create(p.second.get<std::string>("type"));
...
bool PosetsProduct::IsIndependent(PosetItemsMatrix const& idealBase,
                                  PosetItemsMatrix const& filterBase,
                                  PosetItems const& lower,
                                  PosetItems const& upper) const {
    return Preceq(lower, upper) && IsIdealIndependent(idealBase, upper) && IsFilterIndependent(filterBase, lower);
}

bool PosetsProduct::IsMaxMinIndependent(PosetItemsMatrix const& idealBase,
                                        PosetItemsMatrix const& filterBase,
                                        PosetItems const& lower,
                                        PosetItems const& upper) const {
    return Preceq(lower, upper) && IsMaximalIndependent(idealBase, upper) && IsMinimalIndependent(filterBase, lower);
}
...
PosetItemsMatrix PosetsProduct::LoadItems(std::string const& filename, Options const& options) {
    ...
    for (int j = 0; j != n; ++j) {
        ...
        for (int i = 0; i != m; ++i) {
            column[i] = _posets[j]->FromString(csvColumn.values[i + 1].begin, csvColumn.values[i + 1].end);
        }
    }
...
void PosetsProduct::GetSupportItems(PosetItemsMatrix const& dataset,
...
void PosetsProduct::GetUnsupportItems(PosetItemsMatrix const& dataset,
                                      PosetItems const& items,
                                      std::vector<int>& unsupport) const {
    unsupport.clear();
    for (int i = 0, m = dataset.GetHeight(); i != m; ++i)
        if (!Preceq(items, dataset[i]))
...
void PosetsProduct::UpdateUnsupportItems(PosetItemsMatrix const& dataset,
                                         PosetItems const& items,
                                         std::vector<int>& unsupport) const {
    remove_if(unsupport, [this, &items, &dataset](int i) { return Preceq(items, dataset[i]); });
}

void PosetsProduct::UpdateSupportItems(PosetItemsMatrix const& dataset,
                                       PosetItems const& items,
                                       std::vector<int>& support) const {
    remove_if(support, [this, &items, &dataset](int i) { return !Preceq(items, dataset[i]); });
}
