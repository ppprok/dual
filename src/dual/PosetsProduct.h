...
//        PosetsProduct, (_bindings)(_posets)(_reversedPosets));
    ...
    bool IsIndependent(
        PosetItemsMatrix const& idealBase, 
        PosetItemsMatrix const& filterBase,
        PosetItems const& lower, PosetItems const& upper) const;

    // Check independency of interval and it maximality
    bool IsMaxMinIndependent(
        PosetItemsMatrix const& idealBase, 
        PosetItemsMatrix const& filterBase,
        PosetItems const& lower, PosetItems const& upper) const;
    ...
    */
	...
	template <typename Item>
	int UpdateSupportLevel( PosetItemsMatrix const& dataset, Item const& item, Indecies const& support, int threshold ) const
	{
		int s = 0;
		for (int i : support)
		{
	...
	template <typename Item>
	int UpdateUnsupportLevel( PosetItemsMatrix const& dataset, Item const& item, Indecies const& unsupport, int threshold ) const
	{
		int s = 0;
		for (int i : unsupport)
		{
    ...
    */
    ...
    std::string ToString(PosetItemsMatrix const& matrix, std::string const& separator = ",", std::string rowSep = "\n") const;
