...
ReversedPoset::ReversedPoset( IPosetPtr const& poset ) 
    :_inner(poset)
{
    assert(_inner);
}

ReversedPoset::ReversedPoset()
...
void ReversedPoset::GetIndependentIntervals( PosetItems const& A, PosetItems const& B, PosetIntervals& independent ) const
{
...
void ReversedPoset::Register()
{
    Registrator<IPosetPtr>::Instance().RegisterFactory
        ("ReversePoset", [](){return std::make_shared<ReversedPoset>();});
}
