...
void SubsetsPoset::Register() {
    Registrator<IPosetPtr>::Instance().RegisterFactory("SubsetsPoset",
                                                       []() { return std::make_shared<SubsetsPoset>(); });
}
...
void SubsetsPoset::GetIndependentIntervals(PosetItems const& A,
                                           PosetItems const& B,
...
bool SubsetsPoset::Prec(PosetItem const& left, PosetItem const& right) const {
    return left.value.subset != right.value.subset && ((left.value.subset & right.value.subset) == left.value.subset);
}
